 ...
 * (MSCKF)](https://ieeexplore.ieee.org/document/4209642) @cite Mourikis2007ICRA which leverages inertial and visual
 * feature information. We want to stress that this is **not** a "vanilla" implementation of the filter and instead has
 * many more features and improvements over the original. In additional we have a modular type system which allows us to
 * initialize and marginalizing variables out of state with ease. Please see the following documentation pages for
 * derivation details:
 *
 ...
 *
 * We suggest those that are interested to first checkout the State and Propagator which should provide a nice
 * introduction to the code. Both the slam and msckf features leverage the same Jacobian code, and thus we also

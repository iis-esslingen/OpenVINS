...
TrackBase::TrackBase(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numfeats, int numaruco,
                     bool stereo, HistogramMethod histmethod)
...
void TrackBase::display_active(cv::Mat& img_out, int r1, int g1, int b1, int r2, int g2, int b2, std::string overlay)
{
  ...
  for (auto const& pair : img_last_cache)
  {
    ...
    for (size_t i = 0; i < pts_last_cache[pair.first].size(); i++)
    {
      ...
      // cv::FONT_HERSHEY_SIMPLEX,0.5,cv::Scalar(0,0,255),1,cv::LINE_AA); Draw rectangle around the point
    ...
    if (overlay == "")
    {
      cv::putText(img_temp, "CAM:" + std::to_string((int)pair.first), txtpt, cv::FONT_HERSHEY_COMPLEX_SMALL,
                  (is_small) ? 1.5 : 3.0, cv::Scalar(0, 255, 0), 3);
    }
    else
    {
      cv::putText(img_temp, overlay, txtpt, cv::FONT_HERSHEY_COMPLEX_SMALL, (is_small) ? 1.5 : 3.0,
                  cv::Scalar(0, 0, 255), 3);
    }
    // Overlay the mask
    ...
    img_temp.copyTo(
        img_out(cv::Rect(max_width * index_cam, 0, img_last_cache[pair.first].cols, img_last_cache[pair.first].rows)));
    index_cam++;
  }
}
  ...
  for (auto const& pair : img_last_cache)
  {
    ...
    for (size_t i = 0; i < ids_last_cache[pair.first].size(); i++)
    {
      ...
      for (size_t z = feat.uvs[pair.first].size() - 1; z > 0; z--)
      {
        ...
        {
          // cv::putText(img_out0, std::to_string(feat->featid), pt_c, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0,
          // 255), 1, cv::LINE_AA); cv::circle(img_out0, pt_c, 2, cv::Scalar(color,color,255), CV_FILLED);
        }
      }
    }
    // Draw what camera this is

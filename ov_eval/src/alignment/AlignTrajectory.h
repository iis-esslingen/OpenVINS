 ...
 *
 * Given two trajectories that have already been time synchronized we will compute the alignment transform between the
 * two. We can do this using different alignment methods such as full SE(3) transform, just postiion and yaw, or SIM(3).
 * These scripts are based on the [rpg_trajectory_evaluation](https://github.com/uzh-rpg/rpg_trajectory_evaluation)
 * toolkit by Zhang and Scaramuzza. Please take a look at their [2018 IROS](http://rpg.ifi.uzh.ch/docs/IROS18_Zhang.pdf)
 * paper on these methods.
 */
...
public:
  ...
  static void align_trajectory(const std::vector<Eigen::Matrix<double, 7, 1>>& traj_es,
                               ...
                               Eigen::Vector3d& t, double& s, std::string method, int n_aligned = -1);

protected:
  /**
   * @brief Align estimate to GT using only position and yaw (for gravity aligned trajectories) using only the first
   * poses
   * @param traj_es Estimated trajectory values in estimate frame [pos,quat]
   * @param traj_gt Groundtruth trjaectory in groundtruth frame [pos,quat]
   * @param R Rotation from estimate to GT frame that will be computed
  ...
  /**
   * @brief Align estimate to GT using only position and yaw (for gravity aligned trajectories) using a set of initial
   * poses
   * @param traj_es Estimated trajectory values in estimate frame [pos,quat]
   * @param traj_gt Groundtruth trjaectory in groundtruth frame [pos,quat]
   * @param R Rotation from estimate to GT frame that will be computed
   ...
   * @param n_aligned Number of poses to use for alignment (-1 will use all)
   */
  static void align_posyaw(const std::vector<Eigen::Matrix<double, 7, 1>>& traj_es,
                           const std::vector<Eigen::Matrix<double, 7, 1>>& traj_gt, Eigen::Matrix3d& R,
                           Eigen::Vector3d& t, int n_aligned = -1);
   ...
   * @param n_aligned Number of poses to use for alignment (-1 will use all)
   */
  static void align_se3(const std::vector<Eigen::Matrix<double, 7, 1>>& traj_es,
   ...
   * @param n_aligned Number of poses to use for alignment (-1 will use all)
   */
  static void align_sim3(const std::vector<Eigen::Matrix<double, 7, 1>>& traj_es,
                         const std::vector<Eigen::Matrix<double, 7, 1>>& traj_gt, Eigen::Matrix3d& R,
                         Eigen::Vector3d& t, double& s, int n_aligned = -1);
};

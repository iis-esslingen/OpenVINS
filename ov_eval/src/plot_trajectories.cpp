...
void plot_xy_positions(const std::string& name, const std::string& color,
...
int main(int argc, char** argv)
{
  ...
  if (argc < 3)
  {
    ...
    PRINT_ERROR(RED
                "ERROR: ./plot_trajectories <align_mode> <file_gt.txt> <file_est1.txt> ...  <file_est9.txt>\n" RESET);
    PRINT_ERROR(RED
                "ERROR: rosrun ov_eval plot_trajectories <align_mode> <file_gt.txt> <file_est1.txt> ...  "
                "<file_est9.txt>\n" RESET);
  ...
  for (int i = 2; i < argc; i++)
  {
    ...
    if (i > 2)
    {
      ...
      PRINT_DEBUG("[TRAJ]: q_ESTtoGT = %.3f, %.3f, %.3f, %.3f | p_ESTinGT = %.3f, %.3f, %.3f | s = %.2f\n",
                  q_ESTtoGT(0), q_ESTtoGT(1), q_ESTtoGT(2), q_ESTtoGT(3), t_ESTinGT(0), t_ESTinGT(1), t_ESTinGT(2),
                  s_ESTtoGT);
      ...
      for (size_t j = 0; j < gt_times_temp.size(); j++)
      {
        ...
        pose_ESTinGT.block(3, 0, 4, 1) =
            ov_core::quat_multiply(poses_temp.at(j).block(3, 0, 4, 1), ov_core::Inv(q_ESTtoGT));

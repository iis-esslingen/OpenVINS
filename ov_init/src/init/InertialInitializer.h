 ...
 * The user can request to wait for a jump in our IMU readings (i.e. device is picked up) or to initialize as soon as
 * possible. For state initialization, the user needs to specify the calibration beforehand, otherwise dynamic is always
 * used. The logic is as follows:
 ...
 *
 * The dynamic system is based on an implementation and extension of the work [Estimator initialization in vision-aided
 * inertial navigation with unknown camera-IMU calibration](https://ieeexplore.ieee.org/document/6386235) @cite
 * Dong2012IROS which solves the initialization problem by first creating a linear system for recovering the camera to
 * IMU rotation, then for velocity, gravity, and feature positions, and finally a full optimization to allow for
 * covariance recovery. Another paper which might be of interest to the reader is [An Analytical Solution to the IMU
 * Initialization Problem for Visual-Inertial Systems](https://ieeexplore.ieee.org/abstract/document/9462400) which has
 * some detailed experiments on scale recovery and the accelerometer bias.
 */

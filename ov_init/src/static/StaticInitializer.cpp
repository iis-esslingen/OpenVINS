...
bool StaticInitializer::initialize(double& timestamp, Eigen::MatrixXd& covariance,
                                   std::vector<std::shared_ptr<Type>>& order, std::shared_ptr<IMU> t_imu,
                                   bool wait_for_jerk)
{
  ...
  // Thus, after determining the global to current IMU orientation after initialization, we can propagate the global
  // error into the new IMU pose. In this case the position is directly equivalent, but the orientation needs to be
  // propagated. We propagate the global orientation into the current local IMU frame R_GtoI = R_GtoI*R_GtoG -> H =
  // R_GtoI Eigen::Matrix3d R_GtoI = quat_2_Rot(q_GtoI); covariance(2, 2) = std::pow(1e-4, 2); covariance.block(0, 0, 3,
  // 3) = R_GtoI * covariance.block(0, 0, 3, 3) * R_GtoI.transpose(); covariance.block(3, 3, 3, 3) = std::pow(1e-3, 2) *

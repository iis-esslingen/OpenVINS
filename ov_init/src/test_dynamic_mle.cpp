    ...
    if (hascam)
    {
      if (buffer_timecam != -1)
      {
        ...
        else
        {
          ...
          cpi =
              std::make_shared<ov_core::CpiV1>(params.sigma_w, params.sigma_wb, params.sigma_a, params.sigma_ab, false);
          ...
          state_k1.block(4, 0, 3, 1) = pos_k + vel_k * cpi->DT - 0.5 * cpi->grav * std::pow(cpi->DT, 2) +
                                       ov_core::quat_2_Rot(quat_k).transpose() * cpi->alpha_tau;
          state_k1.block(7, 0, 3, 1) =
              vel_k - cpi->grav * cpi->DT + ov_core::quat_2_Rot(quat_k).transpose() * cpi->beta_tau;
        ...
        for (size_t n = 0; n < buffer_camids.size(); n++)
        {
          ...
          bool is_fisheye =
              (std::dynamic_pointer_cast<ov_core::CamEqui>(params.camera_intrinsics.at(cam_id)) != nullptr);
      ...
      if (map_states.size() % 10 == 0 && map_states.size() > min_num_meas_to_optimize)
      {
        ...
        if (!map_features.empty())
        {
          for (auto& feat : map_features)
          {
            ...
            pos1 << ceres_vars_feat.at(feat.second)[0], ceres_vars_feat.at(feat.second)[1],
        ...
        PRINT_INFO(
            ...
            (int)summary.iterations.size(), map_states.size(), map_features.size(), summary.num_parameters,
        ...
        //        std::cout << sim.get_map().at(featid)[0] << "," << sim.get_map().at(featid)[1] << "," <<
